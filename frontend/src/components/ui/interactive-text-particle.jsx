...
export const ParticleTextEffect = ({
  ...
  const [textBox] = useState({ str: text });
  ...
  class ParticleClass {
    ...
    move(interactionRadius, hasPointer) {
      let moved = false;
  ...
  const dottify = () => {
    ...
    const pixels = data.reduce((arr, _, i, d) => {
      if (i % 4 === 0) {
        arr.push({
          x: (i / 4) % textBox.w,
  ...
  useEffect(() => {
    initialize();
  }, [text, colors, animationForce, particleDensity, width, height]);
